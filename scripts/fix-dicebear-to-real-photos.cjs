#!/usr/bin/env node
// fix-dicebear-to-real-photos.cjs
// 将数据库中所有 DiceBear 卡通 URL 替换为 randomuser.me 真人照片
// 用法：node scripts/fix-dicebear-to-real-photos.cjs
...
async function main() {
  ...
  const botsResult = await tursoQuery(`
    SELECT id, display_name, gender, avatar, avatar_type
    FROM profile
  ...
  if (!botsResult.results || !botsResult.results[0]) {
    console.error('❌ 查询失败:', JSON.stringify(botsResult).slice(0, 200));
    process.exit(1);
  }
