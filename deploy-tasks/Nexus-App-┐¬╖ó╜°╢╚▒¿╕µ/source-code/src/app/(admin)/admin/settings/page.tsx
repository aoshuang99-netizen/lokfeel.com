            ...
            <div key={item.id} className="flex items-start gap-4 p-4 bg-background-tertiary rounded-xl">
              <div className="flex-1">
                ...
                {item.type === "boolean" ? (
                  ...
                  <input type={item.type === "number" ? "number" : "text"} value={item.value} onChange={(e) => handleUpdate(item.id, e.target.value)} className="input-field max-w-xs" />
                )}
              </div>
          ...
          {config.filter((c) => c.key.includes("PRICE")).map((item) => (
            <div key={item.id} className="p-4 bg-background-tertiary rounded-xl">
              ...
              <div className="flex items-center gap-2">
                ...
                <input type="number" value={item.value} onChange={(e) => handleUpdate(item.id, e.target.value)} className="input-field max-w-[150px]" step="0.01" />
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="glass-card p-6">
        ...
        <div className="grid md:grid-cols-4 gap-4">
          <div>
            ...
            <input type="text" value={newKey} onChange={(e) => setNewKey(e.target.value)} placeholder="CONFIG_KEY" className="input-field" />
          </div>
          <div>
          ...
          <div className="md:col-span-2">
            ...
            <div className="flex gap-2">
              <input type="text" value={newDescription} onChange={(e) => setNewDescription(e.target.value)} placeholder="Optional description" className="input-field flex-1" />
