...
type WrappedHandler = (req: NextRequest, context?: any) => Promise<NextResponse>;
...
): (handler: HandlerFunction) => WrappedHandler {
  return (handler: HandlerFunction): WrappedHandler => {
 ...
 */
export function withAdmin(handler: HandlerFunction): WrappedHandler {
  return async (req: NextRequest, context?: any) => {
    // Auth check with getToken fallback
    let session = await auth();
    let userId = (session?.user as any)?.id;
