    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-white/60">Configure platform parameters</p>
        </div>
        <button onClick={handleSave} disabled={isSaving} className="btn-primary flex items-center gap-2">
          {isSaving ? <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Saving...</> : saved ? <><Check className="w-4 h-4" />Saved!</> : <><Save className="w-4 h-4" />Save Changes</>}
        </button>
      </div>

      {/* Matching Settings */}
      <div className="glass-card p-6">
        <h2 className="text-lg font-semibold text-white mb-4">Matching Configuration</h2>
        <div className="space-y-4">
          {config.filter((c) => c.key.includes("MATCH")).map((item) => (
            <div key={item.id} className="flex items-start gap-4 p-4 bg-white/5 rounded-xl">
              <div className="flex-1">
                ...
                {item.type === "boolean" ? (
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" checked={item.value === "true"} onChange={(e) => handleUpdate(item.id, e.target.checked.toString())} className="w-5 h-5 rounded border-white/20 bg-white/5 text-primary" />
                    <span className="text-white">{item.value === "true" ? "Enabled" : "Disabled"}</span>
                  </label>
                ) : (
              ...
              <button onClick={() => handleDelete(item.id)} className="p-2 rounded-lg hover:bg-error/20 text-white/60 hover:text-error"><Trash2 className="w-4 h-4" /></button>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="glass-card p-6">
        ...
        <div className="grid md:grid-cols-4 gap-4">
          <div>
            <label className="block text-sm text-white/60 mb-2">Key</label>
